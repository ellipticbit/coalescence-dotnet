...
$BuildNumber = ([int]$env:BUILD_NUMBER_WINDOWS) + 1
Invoke-WebRequest "https://gitlab.com/api/v4/projects/$($env:CI_PROJECT_ID)/variables/BUILD_NUMBER_WINDOWS" -Headers @{"PRIVATE-TOKEN"=$env:CI_API_TOKEN} -Body @{value=$BuildNumber} -ContentType "application/x-www-form-urlencoded" -Method "PUT" -UseBasicParsing

$appVer = '{0}.{1}.{2}' -f $BuildMajor, $BuildMinor, $BuildNumber
$copyright = 'Copyright © EllipticBit, LLC. {0}, All Rights Reserved.' -f $year

Get-ChildItem -Path .\Windows\ -Filter *.csproj -Recurse -File | ForEach-Object {
